"""
multi_td3_train_logging.py

MultiBoatEnv(여러 척의 배)에서 TD3 정책 "하나"를 모든 에이전트가 공유하며 학습합니다
(parameter sharing, multi_ppo_train_logging.py / multi_ddpg_train_logging.py와 동일한 방식).

TD3와 DDPG의 차이는 td3_continuous_train_logging.py 상단 docstring 참고
(twin critic, delayed policy update, target policy smoothing).
"""
import os
import sys
...
from td3.td3_continuous_train_logging import TD3Agent


if __name__ == "__main__":
    # ===== 실험 설정 =====
    RESULT_PREFIX = "multi_td3_ep600"
    N_AGENTS = 3
    ...
    # ===== 최고 성능 시점 저장 (multi_ppo / multi_ddpg와 동일한 방식) =====
    BEST_WINDOW = 20
    best_metric = -1e9
