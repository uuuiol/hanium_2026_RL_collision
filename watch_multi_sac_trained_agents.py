"""
watch_multi_sac_trained_agents.py

multi_sac_train_logging.py로 학습한 SAC 정책을 불러와서 화면에 실시간으로 보여줍니다.
watch_multi_td3_trained_agents.py와 거의 동일한 절차이나, SAC의 Actor는 확률적(Gaussian)이라
구경(평가)할 때는 매번 다르게 샘플링하지 않고 분포의 평균(tanh(mu))을 결정론적으로 사용합니다.
"""
import time
import numpy as np
...
from sac_model import GaussianActor

# ===== 설정 =====
WEIGHTS_PATH = "multi_sac_ep600_actor_weights_best.weights.h5"
N_AGENTS = 3
MAX_STEPS = 200
...
actor = GaussianActor(env.state_size, env.action_size)
...
def get_greedy_action(state):
    ...
    _, _, det_action = actor.sample(state_in)
